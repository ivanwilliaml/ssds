...
def envfeat(env_df,tg):
    ...
    for d,l in [(1,"1d"),(3,"3d"),(7,"7d"),(14,"14d"),(30,"30d"),(60,"60d")]:
...
exog=(["nama_pos","landcover_name","latitude","longitude","hour_sin","hour_cos","month_sin","month_cos","doy_sin","doy_cos"]
    +MEAN+["rainfall_mm","rainfall_openmeteo_mm"]+[f"rainfall_sum_{l}" for l in ["1d","3d","7d","14d","30d","60d"]]
...
VA_END=CUT+pd.Timedelta(days=242)  # match the real test span (Sep -> May)
tr=train[train["datetime"]<=CUT]; va=train[(train["datetime"]>CUT)&(train["datetime"]<=VA_END)]
print(f"{el()} SEASON-MATCHED backtest (Sep->May): tr={len(tr)} va={len(va)} ({(va['datetime'].max()-va['datetime'].min()).days}d)")
src=tr
...
# ---- ANCHOR strategy comparison (is a seasonal anchor better than ML for far horizon?) ----
seas=va_n["seasonal_lag_1y"].values.astype(float)
clim=va_n["doy_clim_mean"].values.astype(float)
print(f"\n{el()} --- standalone anchors ---")
print(f"{el()} seasonal_lag_1y only  RMSE={rmse(y,seas):.4f}")
print(f"{el()} doy_climatology only  RMSE={rmse(y,clim):.4f}")
print(f"{el()} --- blend last_known(near) -> ANCHOR(far), w=exp(-h/tau) ---")
for anchor_name,anchor in [("ML_direct",direct),("seasonal_1y",seas),("doy_clim",clim)]:
    bb=(1e9,None)
    for tau in [30,45,60,90,120,150,180]:
        w=np.exp(-h/tau); r=rmse(y,w*lk+(1-w)*anchor)
        if r<bb[0]: bb=(r,tau)
    print(f"{el()}   persistence->{anchor_name:<12} best RMSE={bb[0]:.4f} (tau={bb[1]})")
# 3-way: last_known + seasonal + ML, tune coarse with horizon weighting on last_known
print(f"{el()} --- 3-way: w*last_known + (1-w)*(a*seasonal_1y + (1-a)*ML_direct) ---")
b3=(1e9,None)
for tau in [45,60,90,120]:
    w=np.exp(-h/tau)
    for a in [0.0,0.25,0.5,0.75,1.0]:
        far=a*seas+(1-a)*direct
        r=rmse(y,w*lk+(1-w)*far)
        if r<b3[0]: b3=(r,(tau,a))
print(f"{el()}   BEST 3-way RMSE={b3[0]:.4f}  (tau,a)={b3[1]}")
